...
# Requires the shared engine (~/.gimp-bg-remover/venv) installed first.
...
SHARED_VENV="$HOME/.gimp-bg-remover/venv/bin/python3"
...
if [ ! -f "$SHARED_VENV" ]; then
    echo ""
    echo "[!] Shared engine not found at ~/.gimp-bg-remover/venv"
